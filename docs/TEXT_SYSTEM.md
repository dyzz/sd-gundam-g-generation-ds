...
| ID-command box titles | **64 px** registered string budget; on 配属 the three 2-tile-high rows share a worst-case 72-char-tile reservation `0x263..0x2AA` (max 12 tiles/row including the engine's padding/advance), and the right ability bank begins at the non-overlap boundary `0x2AB` (LESSONS §A15) |
